...
def generate_doxygen_xml(app):
    ...
    conda install -y -q gfortran_linux-64 gxx_linux-64 zlib<1.2.11
    R_VER=3.6.0
    wget -q https://cran.r-project.org/src/base/R-3/R-$R_VER.tar.gz
    tar -xzf R-$R_VER.tar.gz
    R-$R_VER/configure --with-x=no --with-readline=no --enable-R-profiling=no --enable-memory-profiling=no --enable-R-shlib=no --enable-R-shlib=no --with-tcltk=no --prefix=$HOME/R
    make
    make install
    """
    try:
